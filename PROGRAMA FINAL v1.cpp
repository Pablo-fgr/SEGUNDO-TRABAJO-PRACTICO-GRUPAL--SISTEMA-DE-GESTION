...
void RegistrarMascota(usuarios PP,mascota &Oscar,int i)
{
	...
	do
	{
		...
		printf("\n\tDia---> ");scanf("%d",&Oscar.FechaNacimiento.Dia);
		printf("\n\tMes---> ");scanf("%d",&Oscar.FechaNacimiento.Mes);
	...
	printf("\nLos datos fueron cargados existosamente... volviendo al menú de asistente\n");
...
void RegistrarTurno(usuarios PP,mascota Oscar,int i)
{
   ...
   if(i==0)
   {
   		printf("\n\tInicie sesión primero porfavor.\n\n");
   		system("pause");system("cls");return;
   }
	else{
		printf("\n\tBienvenido a la sección de turnos %s.\n\n",PP.ApellidoNombre);
		system("pause");system("cls");
	}
	...
	if(p==NULL)
	{
		printf("\n\tLa lista de mascotas esta vacia,primero debe ser cargada.\n");
		system("pause");system("cls");return;
	}
	///////////////////////////////////////////
	fclose(p);
	...
	do
	{
		do
		{
			...
			printf("\n\tDesea cargar otro turno?(1:SI,2:NO).\n");
			printf("OPCION: ");
			scanf("%d",&n);
			system("cls");
			N=0;
		...
		if(n==1)
		{
			...
			if(N==1)
			{
				printf("\n\t---> Mascota encontrada en el archivo!!, cuando desea que sea su turno?\n");
	    		...
	    		printf("\t************************************************************\n\n");
					...
					printf("\n\t---> Dueño de la mascota no encontrado, verifique su DNI porfavor.\n\n");
					system("pause");system("cls");
				}
...
void ListarAtencion(usuarios PP,int i)
{
	...
	if(i==0){
		printf("\n\n\t---> Primero inicie sesion antes de acceder a esta seccion\n");
		system("pause");system("cls");return;
	}
	...
	printf("\n\t---> La lista de turnos atendidos con sus correspondientes veterinarios es:\n ");
	...
	while(!feof(p))
	{
		...
		printf("\n\t---> Lista de atenciones del veterinario %s:\n");
		while(!feof(pp)){
		
		if(vet.Matricula==turn.Matricula){
			printf("\n\t--->%d/%d/%d\n",turn.fecha.Dia,turn.fecha.Mes,turn.fecha.Anio);
...
void AtencionesDeterminadoMes(int n)
{
	...
	{
		printf("\n\n\t---> El archivo de los veterinarios o el de los turnos atendidos esta vacio, por favor carguelos y vuelva a intentar\n");
...
void TopVeterinarios(int n)
{
	...
	if(p==NULL or pp==NULL)
	{
		printf("\n\n\t---> ERORR primero debe cargar Turnos atendidos o veterinarios\n");
		system("pause");system("cls");return;
	}
...
void IniciarSeccion(veterinarios vet,int &b)
{
	...
	while(n==0)
	{
    	...
    	printf("\n\tPara volver al menúi anterior coloque 0  como respuestas.");
...
void VisualizarListaDeEspera(turnos turn,veterinarios vet,int &k)
{
	...
	printf("\n\t*************************** LISTA DE ESPERA DE LOS TURNOS ********************************");  
	
	printf("\n\n\t---> La Lista De Turnos Se Compone De La Siguiente Manera: \n");
	
	while(!feof(p))
	{
		TurnosTotales++;
			printf("\n\n\t--->DNI del dueño: ",turn.Dni_Dueno);
	...
	do
	{
		printf("\n\n\t---> Desea elegir un turno para atender máss adelante?(1:SI,2:NO)\n");
		printf("OPCION: ");
		scanf("%d",&n);
	}while(n!=1 and n!=2);
	...
	{
		do{
			printf("\n\n\t---> Ingrese el turno a elegir: ");scanf("%d",&k);
			fclose(p);
			system("CLS");	
...
void CargarInforme(turnos turn,veterinarios vet,int k)
{
	...
	{
		fclose(p);
		printf("\n\n\t---> Error debe cargar primero Turnos\n");
		system("pause");system("cls");return;
	}
	...
	if(k==0)
	{
		printf("\n\n\t---> ERROR\nUsted no ha seleccionado ningun turno de la lista de espera.");printf("\n\n");system("PAUSE");system("CLS");
		return;
	}
